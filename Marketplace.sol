...
// File: contracts/DreamhubNFTMarketplace.sol


pragma solidity ^0.8.0;
pragma abicoder v2;

contract DreamhubNFTMarketplace is ERC721Holder, Ownable, ReentrancyGuard {
    using EnumerableSet for EnumerableSet.AddressSet;
    ...
    uint256 public royalityFee; // Royality fee which will be rewarded to seller only 1 time
    ...
    mapping(address => mapping(uint256 => bool)) private _royalityFeeOnToken; // First time royality on token
     ...
     * @param _royalityFee: maximum ask price
     */
    constructor(
        address _adminAddress,
        address _treasuryAddress,
        address _WFTMAddress,// "0x07b9c47452c41e8e00f98ac4c075f5c443281d2a"
        ...
        require(_WFTMAddress != address(0), "Operations: WFTM address cannot be zero");
        require(_royalityFee > 0, "Operations: _royalityFee must be > 0");
     ...
     */
    function buyTokenUsingFTM(address _collection, uint256 _tokenId) external payable nonReentrant {
        // Wrap FTM
    ...
    function buyTokenUsingWFTM(
        ...
        IERC20(WFTM).safeTransferFrom(address(msg.sender), address(this), _price);
    ...
    function createAskOrder(
        ...
        bool isRoyalityPaid = _royalityFeeOnToken[_collection][_tokenId];

        _askDetails[_collection][_tokenId] = Ask({
            seller: msg.sender,
            price: isRoyalityPaid ? _askPrice : _askPrice+royalityFee
        });
    ...
    function _buyToken(
        ...
        _royalityFeeOnToken[_collection][_tokenId] = true;
        // Emit event
        emit Trade(_collection, _tokenId, askOrder.seller, msg.sender, _price, netPrice, _withFTM);
    }
